...
sys.stdout.reconfigure(encoding='utf-8')from tkinter import font
import tkinter as tk
from tkinter import scrolledtext, ttk
import threading
import time
...
# 1. KHAI BÁO VỚI WINDOWS ĐỂ ĐỒNG BỘ LOGO XUỐNG TASKBAR
try:  
    myappid = 'mycompany.chessai.assistant.new_force_v2' # Chuỗi ID bất kỳ duy nhất
...
# Nếu chạy ở chế độ ẩn CMD (--windowed), sys.stdout sẽ là None
if sys.stdout is None or isinstance(sys.stdout, io.StringIO) or not hasattr(sys.stdout, 'reconfigure'):
    sys.stdout = SafeStdout()
if sys.stderr is None or isinstance(sys.stderr, io.StringIO) or not hasattr(sys.stderr, 'reconfigure'):
    sys.stderr = SafeStdout()
...
class ChessApp:
    def __init__(self, root):
        ...
        if os.path.exists("assets/logo.png"):
            ...
            # Tham số False giúp ép logo áp dụng lên toàn bộ tiến trình hệ thống
...
# ────────────────────────────────────────────────────────────────
# HÀM XỬ LÝ MÀN HÌNH CHỜ (SPLASH SCREEN)
# ────────────────────────────────────────────────────────────────
def run_loading_progress(splash, progress, status_lbl, on_complete):
    """ Hàm chạy bằng Thread riêng để cập nhật thanh tiến trình không gây đơ giao diện """
    stages = [
        (15, "Đang kiểm tra luồng hệ thống Windows..."),
        (35, "Đang nạp các thư viện WebDriver & Selenium..."),
        (55, "Cấu hình bộ nhớ đệm AI (MCTS + CNN)..."),
        (75, "Đang tối ưu hóa tài nguyên đồ họa render..."),
        (95, "Đang hoàn tất nạp giao diện chính..."),
        (100, "Sẵn sàng!")
    ]
    ...
    for percent, text in stages:
        ...
        splash.update()
        time.sleep(0.3) # Đã tối ưu tốc độ load gọn gàng mượt mà hơn

    splash.destroy()
    on_complete()

def create_splash_screen(root, on_complete):
    splash = tk.Toplevel()
    splash.title("Đang khởi động...")
    splash.configure(bg="#16213e")
    ...
    if os.path.exists("assets/logo.png"):
        try:
            splash_img = tk.PhotoImage(file="assets/logo.png")
            splash.iconphoto(True, splash_img)
            splash.image_ref = splash_img
        except Exception:
            pass

    tk.Label(splash, text="♞ TRỢ LÝ AI CỜ VUA", font=("Segoe UI", 18, "bold"), bg="#16213e", fg="#e94560").pack(pady=(45, 8))
    
    status_lbl = tk.Label(splash, text="Đang khởi tạo hệ thống...", font=("Segoe UI", 10, "italic"), bg="#16213e", fg="#aaa")
    status_lbl.pack(pady=5)

    style = ttk.Style()
    style.theme_use('default')
    style.configure("Splash.Horizontal.TProgressbar", foreground='#00ff88', background='#00ff88', thickness=8, troughcolor='#0f3460', bordercolor='#16213e')
    
    progress = ttk.Progressbar(splash, style="Splash.Horizontal.TProgressbar", orient=tk.HORIZONTAL, length=350, mode='determinate')
    progress.pack(pady=15)

    tk.Label(splash, text="Đang tải cấu hình...", font=("Segoe UI", 8), bg="#16213e", fg="#555").pack(side=tk.BOTTOM, pady=27)

    # ĐÃ SỬA: Truyền tên hàm (nguyên mẫu tham chiếu) thay vì kích hoạt thực thi luôn bằng dấu ngoặc kép ()
    splash.after(200, lambda: threading.Thread(target=run_loading_progress, args=(splash, progress, status_lbl, on_complete), daemon=True).start())


def launch_main_app(root):
    root.deiconify()       # Hiển thị lại Tk() chính 
    ChessApp(root)         # Nạp toàn bộ lớp giao diện chính vào đây


# ────────────────────────────────────────────────────────────────
# LUỒNG KHỞI CHẠY CHÍNH ĐÃ ĐƯỢC TỐI ƯU CẤU TRÚC CHUẨN
# ────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    root = tk.Tk()
    root.withdraw()  # Ẩn cửa sổ tổng khi mới bật lên

    # Gọi màn hình chờ trước, sau khi chạy hết 100% callback launch_main_app sẽ tự mở giao diện chính
    create_splash_screen(root, lambda: launch_main_app(root))
